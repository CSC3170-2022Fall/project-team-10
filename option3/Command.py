class Command:
    def __init__(self):
        self.type = "" #possibile values: 'create table' 'create table as' 'load' 'store' 'insert into' 'print' 'quit' 'exit' 'select'
        self.name = [] # a list for name of tables, all elements are in form of string
        ...
        self.rows = [] #used for insert command, refers to <literal>, in form of string even the original data is number
        # e.g. if the column of the table is ['instructor_id', 'salary', 'instructor_name']
        # then one instance may be ['123', '12000.00', 'Tom']
        self.column = [] # a list of string that store the column name, in form of string
        # used for select and create command, refers to <column name>
        #e.g. ['studemt_id', 'student_name', 'major']
        self.condition = [] # a list of condition classes
